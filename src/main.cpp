...
//Adafruit_ST7735 EsploraTFT = Adafruit_ST7735(cs, dc, mosi, sclk, rst);
...
void TestLED() {  // text RGB LED on Esplora interactively
  ...
  EsploraTFT.fillScreen(ST7735_BLACK);         // clear screen
...
void setup() {
  ...
  EsploraTFT.setTextWrap(false); // Allow text to run off right edge
}
